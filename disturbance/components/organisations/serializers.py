...
class OrganisationCheckSerializer(serializers.Serializer):
    ...
    def validate(self, data):
        ...
        requests = OrganisationRequest.objects.filter(abn=data["abn"], role="employee").exclude(
    ...
    def validate(self, data):
        ...
        if data["exists"]:
            ...
            if can_approve(org, user):
                raise serializers.ValidationError(
                    "Please contact {} to Approve your request.".format(data["first_five"])
                )
        # Check no consultant request is pending approval for an ABN
        if (
            OrganisationRequest.objects.filter(abn=data["abn"], requester=user, role="consultant")
