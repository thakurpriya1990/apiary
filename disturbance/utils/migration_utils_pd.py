...
from disturbance.components.proposals.models import (  # , ProposalOtherDetails, ProposalPark
    ApiarySite,
...
class ApiaryLicenceReader:
    ...
    FROM mgt-command:
        python manage.py apiary_migration_script --filename disturbance/utils/csv/apiary_migration_file_02Jun2022.xlsx
    ...
    def __init__(self, filename):
        ...
        self.proposal_type = (
            ...
            .distinct("name")
            .get(name=self.application_type.name)
        )
    ...
    def _read_excel(self, filename):
        ...
        df["approval_cpc_date"] = pd.to_datetime(
            df["approval_cpc_date"], errors="coerce"
        )
        df["approval_minister_date"] = pd.to_datetime(
            df["approval_minister_date"], errors="coerce"
        )

        df["issue_date"] = df.apply(
            lambda row: (
                row.issue_date
        ...
        df["first_name"] = df["first_name"].apply(
            lambda x: (
                x.lower().capitalize().strip() if not pd.isnull(x) else "No First Name"
            )
        )
        df["last_name"] = df["last_name"].apply(
            lambda x: (
                x.lower().capitalize().strip() if not pd.isnull(x) else "No Last Name"
            )
        )
        df["licencee"] = df["licencee"].apply(
            lambda x: x.strip() if not pd.isnull(x) else "No Licencee Name"
        )
        ...
        # add extra column
        df["licencee_type"] = df["abn"].apply(
            lambda x: "organisation" if x else "individual"
        )
        ...
        for index, row in self.df.groupby("abn").first().iterrows():
            ...
            try:
                ...
                if lo.count() > 0:
                    ...
                    contact_qs = org.contacts.exclude(
                        email__icontains="ledger.dpaw.wa.gov.au"
                    )
                    ...
                    if (
                        row.email
                        and org.contacts.filter(email__icontains=row.email).count() == 0
                    ):
                        # create contact
                    ...
                    if row.email and org.delegates.all().count() == 0:
                        ...
                        delegate = UserDelegation.objects.get_or_create(
                    ...
                    org, created_org = Organisation.objects.get_or_create(
                        organisation=lo
                    )
                    org_contact = self._create_org_contact(row, org)
                    user = EmailUser.objects.get(email=row.email)
        ...
        with transaction.atomic():
            ...
            for index, row in self.df.iterrows():
                try:
                    site_number = None
                    site_number = (
                        int(row.permit_number)
                        if row.permit_number
                        else int(row.licensed_site)
                    )
                    # ApiarySite.objects.filter(id=site_number)
                    if (
                        site_number not in completed_site_numbers
                        and not ApiarySite.objects.filter(id=site_number).exists()
                    ):
                        ...
                        if row.licencee_type == "organisation":
                            ...
                            user = EmailUser.objects.get(email=row.email)
                            approval_affected = self._migrate_approval(
                                data=row,
                                submitter=user,
                                applicant=org,
                        ...
                        if (
                            approval_affected
                            and approval_affected not in approvals_affected
                        ):
                            approvals_affected.append(approval_affected)

                        print(
                            "Permit No/licensed_site: {} migrated".format(site_number)
                        )
    ...
    def _migrate_approval(self, data, submitter, applicant=None, proxy_applicant=None):
        ...
        try:
            site_number = (
                int(data.permit_number)
                if data.permit_number
                else int(data.licensed_site)
            )
        except Exception:
            # import ipdb; ipdb.set_trace()
            print(
                "ERROR: There is no site_number - Must provide a site number in migration spreadsheeet. {e}"
            )

        try:
            expiry_date = (
                data["expiry_date"] if data["expiry_date"] else datetime.date.today()
            )
            start_date = (
                data["start_date"] if data["start_date"] else datetime.date.today()
            )
            issue_date = data["issue_date"] if data["issue_date"] else start_date
            site_status = (
                "not_to_be_reissued"
                if data["status"].lower().strip() == "not to be reissued"
                else data["status"].lower().strip()
            )
        ...
        try:
            ...
            geometry = GEOSGeometry(
                "POINT(" + str(data["latitude"]) + " " + str(data["longitude"]) + ")",
                srid=4326,
            )
            # import ipdb; ipdb.set_trace()
            apiary_site = ApiarySite.objects.create(
                id=site_number, is_vacant=True if site_status == "vacant" else False
            )
            ...
            intermediary_approval_site = ApiarySiteOnApproval.objects.create(
                ...
                if data.approval_cpc_date
                else None,
                approval_minister_date=data["approval_minister_date"]
                if data.approval_minister_date
                else None,
    ...
    def create_licence_pdf(self, approvals_created):
        ...
        print(
            "Total {} approval(s) for creating licence pdf: {}".format(
                len(approvals_migrated), approvals_migrated
            )
        )
...
def create_invoice(proposal, payment_method="other"):
    ...
    line_items = [
        {
            "ledger_description": "Migration Licence Charge Waiver - {} - {}".format(
