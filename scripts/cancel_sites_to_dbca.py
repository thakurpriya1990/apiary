...
Run (dry run first):
    python manage_ds.py shell -c "exec(open('scripts/cancel_sites_to_dbca.py').read())"

Then for real (requires explicit ACTUAL_RUN=true):
    ACTUAL_RUN=true python manage_ds.py shell -c "exec(open('scripts/cancel_sites_to_dbca.py').read())"
"""
...
print("DBCA Approval   : id={}  lodgement={!r}  status={!r}".format(
    dbca_approval.id, dbca_approval.lodgement_number, dbca_approval.status))
print("Source Approval : id={}  lodgement={!r}  status={!r}".format(
...
for site_id in sorted(TARGET_SITE_IDS):
    ...
    try:
        old_link = ApiarySiteOnApproval.objects.get(apiary_site=site, approval=source_approval)
    except ApiarySiteOnApproval.DoesNotExist:
        errors.append("  ERROR: Site {} has no ApiarySiteOnApproval on Approval {}".format(
            site_id, SOURCE_APPROVAL_ID))
        continue

    existing_dbca_link = ApiarySiteOnApproval.objects.filter(
        apiary_site=site, approval=dbca_approval).first()
...
for a in actions:
    if a['action'] == 'create_new_dbca_link':
        print("  Site {}: CREATE new link on Approval {} (site_status='{}'), "
              "set old link {} -> '{}', update latest_approval_link".format(
                  a['site_id'], DBCA_APPROVAL_ID, SITE_STATUS_NOT_TO_BE_REISSUED,
                  a['old_link_id'], SITE_STATUS_TRANSFERRED))
    else:
        print("  Site {}: UPDATE existing DBCA link {} (was '{}') -> '{}', "
              "set old link {} -> '{}', update latest_approval_link".format(
                  a['site_id'], a['dbca_link_id'], a['existing_dbca_status'],
